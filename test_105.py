...
def make_prediction(real, recon, x_t, threshold=0.5, error_fn="sq"):
    ...
    """
    if error_fn == "sq":
        mse = ((recon - real).square() * 2)
    elif error_fn == "l1":
        mse = (recon - real)
    mse_threshold = mse > (threshold * 2)
    #mse_threshold = (mse_threshold.float() * 2) - 1

    #print('max real is '+str(torch.max(real))+ ' ,min real is '+ str(torch.min(real)))
    #print('max recon is '+str(torch.max(recon))+' ,min recon is '+ str(torch.min(recon)))
    #print(torch.max(mse))
...
def output_masked_comparison(sequence, filename, t_distance=250):
    ...
    fig, subplots = plt.subplots(
            len(sequence), 5, constrained_layout=False, figsize=(5, len(sequence)),
            squeeze=False,
        ...
        for plot in range(brain.shape[0]):
            ...
            if plot > 2:
                ...
                if brain[plot].shape[-3] == 3:
                    ...
                    subplots[i][plot].imshow((brain[plot] ).permute(1, 2, 0).cpu().numpy(), cmap=cmap)
            else:
...
def output_masked_comparison2(sequence, filename, t_distance=250):
    """
    sequence is ideally [[x_0,recon,mse,threshold_mse,ground_truth]*4] where
    [x_0,recon,mse,threshold_mse,ground_truth] is a single (5,1,256,256) torch tensor
    or sequence is [x_0,recon,mse,threshold_mse,ground_truth]
    ...
    fig, subplots = plt.subplots(
            len(sequence), 4, constrained_layout=False, figsize=(4, len(sequence)),
            squeeze=False,
            gridspec_kw={'wspace': 0, 'hspace': 0}
            )
    plt.tick_params(
        ...
        for plot in range(brain.shape[0]):
            ...
            if plot > 2:
                ...
                if brain[plot].shape[-3] == 3:
                    ...
                    subplots[i][plot].imshow((brain[plot] + 1).permute(1, 2, 0).detach().cpu().numpy(), cmap=cmap)
            else:
                # img = scale_img(brain[plot])
                # renorm image
                img = (brain[plot] + 1) / 2
                ...
                subplots[i][plot].imshow(
                        img.permute(1, 2, 0).detach().cpu().numpy(),cmap='gray'
                        )
            subplots[i][plot].tick_params(
                    top=False, bottom=False, left=False, right=False,
                    labelleft=False, labelbottom=False
    ...
    # add labels and save
    for i, val in enumerate(
            ["$x_0$", f"$x_{{{t_distance}}}$", "U-net result", "Square Error"]):
        subplots[0][i].set_xlabel(f"{val}", fontsize=6)
        subplots[0][i].xaxis.set_label_position("top")
...
def make_ano_outputs():
    args, output = load_parameters(device, mnist_number)
    in_channels = 1
    if args["dataset"].lower() == "leather":
        in_channels = 3
    if args["channels"] != "":
    ...
    args["Batch_Size"] = 1
    loader_ = dataset.init_mnist_loader(testing_dataset, args, shuffle=True)
    ...
    for i in [DATASET_PATH+'final-outputs/', DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"]),
              DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/normal/', DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/abnormal/']:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    for i in range(100):
        ...
        for k in range(rows):
            ...
            if new[1].to(device) == 0:
                filename = DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/normal/'
            else:
                filename = DATASET_PATH + 'final-outputs/ARGS=' + str(args["arg_num"]) + '/abnormal/'
            ...
            if args["dataset"] == "mnist" or "ddsm":
                output_images, mse_threshold = make_prediction(
                    img, output[-1].to(device),
                    output[t_distance // 2].to(device), threshold=threshold
                )
            '''
        ...
        #temp = os.listdir(f"./mnist/{mnist_number}/final-outputs/ARGS={args['arg_num']}")
        output_masked_comparison(
                predictions, filename + 'attempt='+ str(countnum) + '-' + str(threshold)+ '-predictions.png', t_distance)
        '''
...
def make_ano_outputs2():
    ...
    for i in [DATASET_PATH+'final-outputs/', DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"]),
              DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/normal_check/', DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/abnormal_check/']:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    for i in range(15):
        ...
        for k in range(rows):
            ...
            new = next(loader)
            '''
            while new[1].to(device) != (i % 2):
                new = next(loader)
            '''
            img = new[0].to(device)
            '''
            if new[1].to(device) == 0:
                filename = DATASET_PATH+ 'final-outputs/ARGS='+str(args["arg_num"])+'/normal_check/'
            else:
            ...
            output_images = torch.cat((img, output[t_distance // 2].to(device), est[:1, ...], (est - noisy).square()[:1, ...]))
            predictions.append(
                    output_images
                    )
            sequences.append(output)
    ...
    #list_abnorm = np.array(list_abnorm)


    #plt.figure(figsize=((8,8)))
    #plt.rcParams.update(plt.rcParamsDefault)
    #for i in list_norm:
    #    plt.plot(0, i, marker ='o', c='b')
    #for i in list_abnorm:
    #    plt.plot(0, i, marker='+', c='r')

    #plt.show()

def make_ano_save_output():
    args, output = load_parameters(device, mnist_number)
    in_channels = 1
    if args["dataset"].lower() == "leather":
        in_channels = 3
    ...
    for i in range(200):
        ...
        for k in range(rows):
            ...
            #torchvision.utils.save_image(output[t_distance // 2], filename)
            #torchvision.utils.save_image(output[-1], filename)
...
if __name__ == '__main__':
    ...
    DATASET_PATH = './mnist/' + str(mnist_number) + '/'
