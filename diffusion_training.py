        ...
        torch.save(
                {
                    ...
                    "args":                 args
                    # 'loss': LOSS,
                    }, ROOT_DIR+'model/diff-params-ARGS='+args["arg_num"]+'/params-final.pt'
                )
    else:
        torch.save(
                {
                    ...
                    "ema":                  ema.state_dict(),
                    'loss':                 loss,
                    }, ROOT_DIR+'model/diff-params-ARGS='+args["arg_num"]+'/checkpoint/diff_epoch='+str(epoch)+'.pt'
                )
...
def training_outputs(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    ...
    if save_vids:
        ...
        if epoch % 500 == 0:
            ...
            ani.save(ROOT_DIR+'diffusion-videos/ARGS='+ args["arg_num"]+'/sample-EPOCH='+str(epoch)+'.mp4')
...
def main():
    ...
    """
    # make directories
    for i in [ROOT_DIR+'model/', ROOT_DIR+'diffusion-videos/', ROOT_DIR+'diffusion-training-images/']:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    # load the json args
    with open(ROOT_DIR+'test_args/' + file, 'r') as f:
        args = json.load(f)
    args['arg_num'] = file[4:-5]
    args = defaultdict_from_json(args)

    # make arg specific directories
    for i in [ROOT_DIR+'model/diff-params-ARGS='+args["arg_num"],
              ROOT_DIR+'model/diff-params-ARGS='+args["arg_num"]+'/checkpoint',
              ROOT_DIR+'diffusion-videos/ARGS='+args["arg_num"],
              ROOT_DIR+'diffusion-training-images/ARGS='+args["arg_num"]]:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    if args["dataset"].lower() == "cifar":
        ...
        training_dataset, testing_dataset = dataset.init_mnist(ROOT_DIR)
        training_dataset_loader_ = dataset.init_mnist_loader(training_dataset, args)
        testing_dataset_loader_ = dataset.init_mnist_loader(testing_dataset, args)
        training_dataset_loader = dataset.cycle(training_dataset_loader_)
        testing_dataset_loader = dataset.cycle(testing_dataset_loader_)

    elif args["dataset"].lower() == "ddsm":
        training_dataset, testing_dataset = dataset.init_DDSM(ROOT_DIR)
        training_dataset_loader = dataset.init_DDSM_loader(training_dataset, args)
        testing_dataset_loader = dataset.init_DDSM_loader(testing_dataset, args)
    ...
    if resume:
        if resume == 1:
            ...
            file_dir = ROOT_DIR+'model/diff-params-ARGS='+str(args["arg_num"])+'/params-final.pt'
            loaded_model = torch.load(file_dir, map_location=device)
    ...
    for file_remove in os.listdir(ROOT_DIR+'/model/diff-params-ARGS='+args["arg_num"]+'/checkpoint'):
        os.remove(os.path.join(ROOT_DIR+'./model/diff-params-ARGS=' + args["arg_num"] + '/checkpoint', file_remove))
    os.removedirs(ROOT_DIR+'./model/diff-params-ARGS='+args["arg_num"]+'/checkpoint')
