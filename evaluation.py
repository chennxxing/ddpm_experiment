...
def testing(testing_dataset_loader, diffusion, args, ema, model):
    ...
    for i in [*range(100, args['sample_distance'], 100)]:
        ...
        if args["dataset"] == "cifar" or args["dataset"] == "carpet" or args["dataset"] == "mnist":
    ...
    for epoch in range(test_iters // args["Batch_Size"] + 5):
        data = next(testing_dataset_loader)
        if args["dataset"] != "cifar" and args["dataset"] != "mnist" and args["dataset"] != "ddsm":
            x = data["image"]
            x = x.to(device)
        else:
            # cifar outputs [data,class]
    ...
    for epoch in range(test_iters // args["Batch_Size"] + 5):
        data = next(testing_dataset_loader)
        if args["dataset"] != "cifar" and args["dataset"] != "mnist":
            x = data["image"]
            x = x.to(device)
        else:
            # cifar outputs [data,class]
