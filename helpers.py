...
def load_checkpoint(param, use_checkpoint, device, number):
    """
    ...
    if not use_checkpoint:
        return torch.load(f'./mnist/{number}/model/diff-params-ARGS={param}/params-final.pt', map_location=device)
    else:
        print("load_checkpoint Error")
        '''
...
def load_parameters(device, number):
    """
    ...
    for param in params:
        if param.isnumeric():
            output = load_checkpoint(param, use_checkpoint, device, number)
