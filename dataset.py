...
def init_mnist(ROOT_DIR):
    training_dataset = MnistDataset(ROOT_DIR, train=True)
    testing_dataset = MnistDataset(ROOT_DIR, train=False)
    return training_dataset, testing_dataset

def init_mnist_loader(mnist_dataset, args, shuffle=True):
    dataset_loader = cycle(
        torch.utils.data.DataLoader(
            mnist_dataset,
...
def init_DDSM(ROOT_DIR):
    training_dataset = DDSMDataset(ROOT_DIR, train=True)
    testing_dataset = DDSMDataset(ROOT_DIR, train=False)
    return training_dataset, testing_dataset

def init_DDSM_loader(ddsm_dataset, args, shuffle=True):
    dataset_loader = cycle(
        torch.utils.data.DataLoader(
            ddsm_dataset,
            batch_size=args['Batch_Size'],
...
class MnistDataset(Dataset):
    ...
    def __init__(self, ROOT_DIR, transform=None, train=True):
        if train:
            ...
            self.image_input = np.load(ROOT_DIR + "normal_test.npy", allow_pickle=True)
    ...
    def __len__(self):
        return len(self.label_input)

    def __getitem__(self, idx):
        if len(self.image_input[idx, :]) == 784:
            image = self.image_input[idx, :].reshape((28,28))
        else:
            image = self.image_input[idx, :].reshape((32, 32))

        if self.transform:
            image = self.transform(image)
        label = self.label_input[idx]
...
class DDSMDataset(Dataset):
    "my style MNIST dataset"
    def __init__(self, ROOT_DIR, transform=None, train=True):
        self.filenames = [ROOT_DIR+'image/'+i for i in os.listdir(ROOT_DIR+'image/')]
        if train:
            self.image_path = ROOT_DIR + 'image/'
            self.label_path = ROOT_DIR + 'label/'
        else:
            self.image_path = ROOT_DIR + 'test_image/'
            self.label_path = ROOT_DIR + 'test_label/'
    ...
    def __getitem__(self, idx):
        image = cv2.imread(self.image_path+str(idx)+'.png')[:,:,0]

        if self.transform:
            image = self.transform(image)
        label = cv2.imread(self.label_path+str(idx)+'.png')[:,:,0]
